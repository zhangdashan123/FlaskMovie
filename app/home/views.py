...
# 修改文件名称
def change_filename(filenmae):
    fileinfo = os.path.splitext(filenmae)
    filenmae = datetime.datetime.now().strftime("%Y%m%d%H%M%S") + str(uuid.uuid4().hex) + fileinfo[-1]
    return filenmae
...
def tm():
    ...
    if request.method == "POST":
        ...
        msg = {
            ...
            "type": data["type"],
            "ip": request.remote_addr,
            "_id": str(datetime.datetime.now().strftime("%Y%m%d%H%M%S")) + str(uuid.uuid4()),
            "player": data["id"]
        }
        res = {
            "code": 1,
